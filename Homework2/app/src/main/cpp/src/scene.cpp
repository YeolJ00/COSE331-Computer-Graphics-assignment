...
void Scene::setup(AAssetManager* aAssetManager) {
    ...
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S, GL_MIRRORED_REPEAT);// to be tested
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);// to be tested
    //└──────────────────────────────────────────┘
...
void Scene::update(float deltaTime) {
    ...
    Scene::teapot->material->threshold += 0.1*deltaTime;
    if(Scene::teapot->material->threshold > 1.0f) Scene::teapot->material->threshold = 0.0f;
    //└──────────────────────────────────────────┘
